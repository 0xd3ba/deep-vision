...
#           I decided to implement the VGG-16 version of this, was having dimension issues
#           with AlexNet (need to check this later on)
#
# NOTE: I'm not trying to bring the image sizes to same scale by random cropping to fixed dimension
#           Although doing so enables for training with batches (more efficient way), my goal is not efficiency
#           but to understand the implementation. So the images will be of the original dimension and hence, training
#           will be on batch sizes of 1, i.e. per image.
...
class FCN(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        # PART-2: The Fully-Convolutional block
        self.full_conv_blk = nn.Sequential(
            nn.Conv2d(in_channels=512, out_channels=n_classes, kernel_size=(1, 1))  # Output size: (3, 7, 7)
        )

        self.decoder_conv_blk_5 = DecoderBlock3x3(n_layers=3, in_channels=n_classes, out_channels=512)   # Output shape: (512, 7, 7)
